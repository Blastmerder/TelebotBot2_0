...
bot.set_my_commands([
    ...
    types.BotCommand("set_time_masage", "Вам дадут инструкции"),
...
def send_hello():
    global id
    bot.send_message(id, f'Сейчас в городе {weather_forcast.get_data().json()["name"]} {weather_forcast.get_data().json()["weather"][0]["description"]}.')
...
def callback(call):
    weather_forcast.set_units("metric")
...
def callback(call):
    weather_forcast.set_units("imperic")
...
def get_weather(message):
    bot.send_message(message.chat.id, f'Сейчас в городе {weather_forcast.get_data().json()["name"]} {weather_forcast.get_data().json()["weather"][0]["description"]}.')
...
def set_city(message):
    ...
    if last_command == "start":
        if weather_forcast.set_city(message.text):
        ...
        if weather_forcast.set_lang(message.text):
...
job = schedule.every().day.at(time_seten).do(send_hello)
